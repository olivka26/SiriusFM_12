...
class MCOptionHedger1D{
    class OHPathEval{
        ...
        double m_maxPnL;
    public:
        OHPathEval(Option <AssetClassA,AssetClassB>const* a_option,A_Provider const  * a_irpA,B_Provider const  * a_irpB,double a_C0, DeltaFunc const* a_deltaFunc,double a_DeltaAcc)
        :m_option(a_option),
        m_irpA(a_irpA),
        m_irpB(a_irpB),
        m_ratesA(nullptr),
        ...
        m_minPnL(INFINITY),
        m_maxPnL(-INFINITY)
        {assert(m_option!=nullptr && m_deltaFunc!=nullptr && m_DeltaAcc>=0 && m_irpA !=nullptr && m_irpB !=nullptr);}
        void operator()(long a_L, long a_PM, double const * a_paths, double const * a_ts){
            ...
            for(long p=0;p<a_PM;++p){
                ...
                m_sumPnL+=PnL;
                m_sumPnL2+PnL*PnL;
                m_minPnL = std::min<double>(m_minP0, PnL);
        ...
        std::tuple<double,double,double,double> GetStats()const{
           ...
           // double err=(px!=0)? sqrt(var)/fabs(px) : sqrt(var);
            return std::make_tuple(sqrt(var), m_minPnL, m_maxPnL);
        }
        ~OHPathEval(){
            delete[] (m_ratesA);
            delete[] (m_ratesB);
